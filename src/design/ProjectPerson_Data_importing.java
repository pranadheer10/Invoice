...
import ServiceDB.ProjectPersonService;

public class ProjectPerson_Data_importing {
	...
	public ProjectPerson_Data_importing() {
		frame = new JFrame();
		...
		liftLable = new JLabel("Import Project_Person Data");
		viewLable = new JLabel("View");
		import_B = new JButton("Import");
		importLable = new JLabel("Click On Import To Add ProjectPerson Record");
		...
		mainLable = new JLabel("ProjectPerson Data Importing");
		...
		try {
			for (ProjectPersonDM projectPersonDM : projectPersonService
				...
				model.addElement(projectPersonDM.dataprint2());
			}
		} catch (Exception e) {
		...
		frame.setTitle("Project Person Data Importing");
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (dataArea.getSelectedValue() != null) {
				try {
					...
					String projectnumberS = arrdd[0];
					String projectnameS = arrdd[1];

					int x = JOptionPane.showConfirmDialog(null,
							"Are you want to delete this Item ?");
					if (x == 0) {
						boolean checkresult = deleteRecord(projectnumberS, projectnameS);
						if (checkresult) {
							...
							ProjectPersonService projectpersonService = new ProjectPersonService();
							try {
								for (ProjectPersonDM projectpersonDM : projectpersonService.GetAllDataListfromDB()) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (dataArea.getSelectedValue() != null) {
				try {
					...
					ProjectPersonDM projectpersonDM0 = GetRecordFromDB(projectnumberS, projectnameS) ;
					if (x == 0 && projectpersonDM0 !=null) {
						frame.setVisible(false);
						new UpdateProjectPerson(projectpersonDM0);
					}
	...
	public boolean deleteRecord(String pnumber, String pname) {
		try {
			java.sql.Statement st = Conn.getconnection().createStatement();

			st.executeUpdate("delete from `database_task03`.`projetc_person` where ProjectNumber = '"
					+ pnumber + "' and ProjectName = '" + pname + "'");
	...
	public ProjectPersonDM GetRecordFromDB(String pnumber, String pname) {
		int number_row = 0;
		try {
			PreparedStatement st = (PreparedStatement) Conn.getconnection()
					.prepareStatement(
							"SELECT * from `database_task03`.`projetc_person` where ProjectNumber = '"
									+ pnumber + "' and ProjectName = '" + pname + "'");
			...
			if (number_row >= 1) {
				...
				while (result.next()) {
					...
					projectpersonDM00.setProject_Number(result.getString("ProjectNumber"));
					projectpersonDM00.setProject_Name(result.getString("ProjectName"));
					projectpersonDM00.setID(result.getInt("idprojetc_person"));

					return projectpersonDM00;
